...
- 强制开场白（必须作为全文第一句话，直接开始，绝对不准加标题）：
  “在【具体时代背景】下，一个【底层的特定身份】偶然吃上一口【被阶级垄断的顶级美食】到底有多爽？你叫【名字】……”
- 无缝剧情指令（严禁分点作答，直接写成连贯的小说自然段）：
  开场后，先用约250字写极其恶劣的劳作环境和肉体濒临停机的极度饥饿；接着用约250字写出极度合理的获取途径，重点描写闻到气味时不受大脑控制狂咽口水的本能；然后用约600字写子弹时间偷食，放慢动作十倍描写咀嚼音效、物理材质、触感以及颅内高潮，这是全篇高潮；最后用约100字写吃干抹净原路逃离，留下满足的长叹，果断收尾。
"""
    elif mode == "脑洞爽文":
        mode_rules = """
【模式：脑洞爽文（降维打击）】
