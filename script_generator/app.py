...
5. 时代常识绝对锁定（防关公战秦琼）：所有行为、求生手段、环境建筑和阶级互动必须【绝对严谨地符合】所设定的时代背景与社会常识！如果用户未提供具体场景让AI自行补全，生成的场景必须【百分之百贴合当前时代】（如1970年浙江绝不能出现金字塔）。在【脑洞爽文】模式下，唯一的跨时代/科幻元素【只能是外挂物品本身】，其余一切（建筑、人物认知、社会运转）必须极度写实，决不允许胡编乱造！
"""
